  ...
  useEffect(() => {
    if (stats?.minutesPlayed) {
      ...
      const animate = (currentTime) => {
        ...
        start = Math.round(progress * targetPercentage);
    ...
    <div className="max-w-6xl mx-auto my-4 sm:my-8 md:my-12 p-4 sm:p-6 md:p-8 bg-gradient-to-br from-gray-100 to-gray-200 rounded-3xl shadow-md">
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start mb-6 md:mb-8">
        ...
        <div className="text-center md:text-left">
          <Title
            level={1}
            className="mb-2 text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800"
          >
          ...
          <div className="flex items-center justify-center md:justify-start mb-2 sm:mb-4">
            {flagImage && (
              <img
                ...
                className="w-6 h-4 sm:w-8 sm:h-6 mr-2 sm:mr-3 shadow-sm"
              />
            )}
            <Text className="text-lg sm:text-xl text-gray-600">
              {player?.nationality || "Nepoznato"}
            </Text>
          </div>
          ...
          <Text className="text-base sm:text-lg text-gray-700 block mb-2 sm:mb-4">
            {player?.club?.name || "Nepoznati klub"},{" "}
            {player?.club?.place || ""}
          </Text>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8 md:mb-12">
        ...
        />
        <StatCard
          icon={<FaSquareFull className="text-yellow-500" />}
          title="Žuti kartoni"
          value={stats.yellowCards}
          color="black"
        />
        <StatCard
          icon={<FaSquareFull className="text-red-500" />}
